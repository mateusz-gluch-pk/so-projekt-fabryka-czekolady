...
int main(int argc, char **argv) {
    ...
    // Setup Logger
    LoggerService logger("t5", TEST_LOG_LEVEL, true);
    ...
    // double warehouse
    auto ia = warehouses.create("t5a", 1);
    if (ia == nullptr) return 1;
    auto ib = warehouses.create("t5b", 1);
    if (ib == nullptr) return 1;

    // Setup Deliverers
    ItemTemplate a("t5a", 1, TEST_DELIVERER_DELAY);
    auto d = deliverers.create("t5-deliverer", a, warehouses);
    if (d == nullptr) return 1;

    // Setup Worker
    std::vector<std::unique_ptr<IItem>> in;
    in.push_back(new_item("t5a", 1));
    auto r = std::make_unique<Recipe>(std::move(in), new_item("t5b", 1), TEST_WORKER_DELAY);
    auto w = workers.create("t5-worker", std::move(r), warehouses);
    if (w == nullptr) return 1;

    usleep(10*TEST_TICK);
    sv.stop_warehouses();
    while (warehouses.get("t5a") != nullptr) {
        usleep(TEST_TICK);
    }
